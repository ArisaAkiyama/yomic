    ...
    public class LibraryService
    {
             ...
             if (existing != null)
             {
                 ...
                 if (deleteFiles)
                 {
                     try 
                     {
                        ...
                        var mangaDir = System.IO.Path.Combine(appData, "MyMangaApp", "Downloads", existing.Source.ToString(), safeTitle);
        ...
        public async Task DeleteChapterDownloadAsync(Manga manga, Chapter chapter)
        {
            try
            {
                ...
                var chapterDir = System.IO.Path.Combine(appData, "MyMangaApp", "Downloads", manga.Source.ToString(), safeMangaTitle, safeChapterName);
        ...
        public async Task ClearDatabaseAsync()
        {
            try
            {
                ...
                var appData = System.Environment.GetFolderPath(System.Environment.SpecialFolder.LocalApplicationData);
                var downloadsFolder = System.IO.Path.Combine(appData, "MyMangaApp", "Downloads");
                var coversFolder = System.IO.Path.Combine(appData, "MyMangaApp", "covers");
